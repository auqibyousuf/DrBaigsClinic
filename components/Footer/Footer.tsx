...
const Footer = () => {
  ...
  if (pathname?.startsWith('/clinic-office')) {
    return null;
  }
              ...
              {Array.from({ length: Math.ceil(services.length / 6) }).map((_, columnIndex) => {
                ...
                return (
                  <div key={`column-${columnIndex}`} className="flex flex-col  space-y-2">
                    {columnServices.map(
                      (service: { name: string; href: string }, itemIndex: number) => {
                        const index = columnIndex * 6 + itemIndex;
                        return (
                          <Link
                          ...
                          >
                            <span className="flex items-center">
                              <svg
                                className="w-3 h-3 mr-1.5 opacity-1 flex-shrink-0"
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                              >
